...
export async function getGoCardlessMappingsForCustomerId(goCardlessCustomerId) {
  await ensureGoCardlessMappingTable();
  ...
  const { rows } = await pgPool.query(
    ...
    `,
    [String(goCardlessCustomerId || "").trim().toUpperCase()]
  );
...
export async function getUniqueHaloClientByName(haloClientName) {
  await ensureGoCardlessMappingTable();
  const normalised = normaliseName(haloClientName);
  if (!normalised) return { client: null, matchCount: 0 };
  ...
  const { rows } = await pgPool.query(
    `
      ...
      WHERE xero_contact_guid IS NOT NULL
        AND lower(regexp_replace(trim(halo_client_name), '\\s+', ' ', 'g')) = $1
  ...
  return {
    client: rows.length === 1 ? rows[0] : null,
    matchCount: rows.length
  };
}

export async function getHaloClientsByExactName(haloClientName) {
  await ensureGoCardlessMappingTable();
  const normalised = normaliseName(haloClientName);
