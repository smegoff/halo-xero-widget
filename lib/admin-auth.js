...
async function sendLockoutAlert({ user, failedCount, lockoutMinutes, ipAddress }) {
...
async function recordFailedLoginAttempt({ user, failureReason, ipAddress, userAgent }) {
  ...
  await recordLoginAudit({
    ...
    success: false,
    failureReason: shouldLock ? `locked_after_${failureReason}` : failureReason,
    ipAddress,
    userAgent
  });

  if (shouldLock) {
    sendLockoutAlert({ user, failedCount, lockoutMinutes, ipAddress }).catch(err => {
      console.warn("Admin lockout alert failed:", err.response?.status || err.message);
    });
  }

  return { failedCount, shouldLock, lockoutMinutes };
}

async function recordSuccessfulLogin({ user, ipAddress, userAgent, mfaStep = null }) {
  await pgPool.query(
    `
      UPDATE halo.admin_user
...
export async function authenticateAdminLogin({ username, password, ipAddress, userAgent }) {
  ...
  if (!passwordOk) {
    const { shouldLock, lockoutMinutes } = await recordFailedLoginAttempt({
      user,
  ...
  return recordSuccessfulLogin({ user, ipAddress, userAgent });
}

export async function completeAdminMfaLogin({ userId, code, ipAddress, userAgent }) {
  await ensureAdminAuthTables();
  ...
  const user = rows[0];
  if (!user || !user.is_active || !user.mfa_enabled_at || !user.mfa_secret) {
    return { ok: false, message: "MFA challenge is no longer valid." };
  }
  ...
  const verification = verifyTotpCode({
    ...
    code,
    lastUsedStep: user.mfa_last_used_step
  });
  if (!verification.ok) {
    const { shouldLock, lockoutMinutes } = await recordFailedLoginAttempt({
      user,
  ...
  return recordSuccessfulLogin({
    ...
    ipAddress,
    userAgent,
    mfaStep: verification.step
  });
}

export async function beginAdminMfaEnrollment(userId) {
  await ensureAdminAuthTables();
  ...
  const { rows } = await pgPool.query(
    `
      ...
      RETURNING id, username, mfa_enabled_at
    `,
    [userId, secret]
  );
  const user = rows[0];
...
export async function confirmAdminMfaEnrollment({ userId, code }) {
  ...
  const { rows } = await pgPool.query(
    `
      SELECT id, username, mfa_pending_secret
      FROM halo.admin_user
      WHERE id = $1 AND is_active = true
      LIMIT 1
    `,
  ...
  if (!user?.mfa_pending_secret) {
    throw new Error("Start MFA setup before confirming a code.");
  }
  ...
  if (!verification.ok) {
    throw new Error("Authenticator code was not valid.");
  }
  ...
  await pgPool.query(
    `
      ...
      WHERE id = $1
    `,
    [user.id, verification.step]
  );
}

export async function resetAdminUserMfa(userId) {
  await ensureAdminAuthTables();
  await pgPool.query(
    `
      UPDATE halo.admin_user
...
export async function getAdminUserById(userId) {
  ...
  const { rows } = await pgPool.query(
    `
      SELECT id, username, role, is_active, mfa_enabled_at
      FROM halo.admin_user
      WHERE id = $1
      LIMIT 1
    `,
...
export function createMfaPendingSession(user) {
  return {
    userId: user.id,
    username: user.username,
    role: user.role,
...
export function isMfaPendingSessionValid(pending) {
  return Boolean(pending?.userId && pending.expiresAt && pending.expiresAt > Date.now());
}
