...
function buildTeamsPayload({ title, severity = "warning", summary, facts = [], actionUrl = "", actionTitle = "Open Admin", actions = [] }) {
  ...
  const factSet = facts
    ...
    .map(fact => ({
      ...
      value: cleanText(fact.value, "Not available").slice(0, 1000)
    }));
  ...
  const actionButtons = actions
    ...
    .map(action => ({
      type: "Action.OpenUrl",
      title: cleanText(action.title, "Open").slice(0, 40),
      url: action.url
    }));

  if (!actionButtons.length && actionUrl) {
    actionButtons.push({
      type: "Action.OpenUrl",
      title: cleanText(actionTitle, "Open Admin").slice(0, 40),
      url: actionUrl
    });
  }

  if (actionButtons.length) {
    content.actions = actionButtons;
  }
